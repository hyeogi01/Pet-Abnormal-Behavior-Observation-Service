"""
dog_abnormal_omni_test.py
────────────────────────────────────────────────────────────────────
강아지 이상 증상 분류 모델 테스트

[클래스]
  Eyes 9클래스 (백내장 제거, 각막질환 통합):
    dog_normal / dog_결막염 / dog_각막질환 / dog_색소침착성각막염
    dog_안검내반증 / dog_안검염 / dog_안검종양 / dog_유루증 / dog_핵경화
...
사용법:
  python dog_abnormal_omni_test.py
  python dog_abnormal_omni_test.py --model_path dog_abnormal_omni_best.pth --device cuda:0
────────────────────────────────────────────────────────────────────
"""
...
DEFAULT_CKPT = "dog_abnormal_omni_best.pth"
IMG_SIZE     = 384
BATCH_SIZE   = 64
NUM_WORKERS  = 24

def parse_args():
    p = argparse.ArgumentParser(description="Dog Abnormal Omni — Test")
...
def plot_confusion_matrix(preds, labels, class_list, title, save_path):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(max(12, n * 1.1 + 2), max(8, n + 1)))
    for ax, data, fmt, ttl in zip(axes, [cm, cm_norm], [".0f", ".2f"],
        ...
        ax.set_xticks(range(n)); ax.set_xticklabels(class_list, rotation=45, ha="right", fontsize=7)
        ax.set_yticks(range(n)); ax.set_yticklabels(class_list, fontsize=7)
        ax.set_xlabel("Predicted"); ax.set_ylabel("Actual"); ax.set_title(f"{title} — {ttl}")
        thresh = data.max() / 2.0
        for i in range(n):
            for j in range(n):
                ax.text(j, i, format(data[i, j], fmt), ha="center", va="center", fontsize=6,
...
def save_summary_plot(results, class_map, avg_acc, best_val_acc, best_epoch):
    ...
    for ax, (task, res), color in zip(axes, results.items(), ["steelblue", "mediumpurple"]):
    ...
    plt.suptitle(f"Dog Abnormal Omni — Test Summary\n"
                 f"Test Avg: {avg_acc*100:.2f}%  |  Best Val: {best_val_acc*100:.1f}% (Ep {best_epoch})\n"
                 f"[Inference mode: feat=zeros, no JSON input]",
                 fontweight="bold", fontsize=11)
    plt.tight_layout()
    plt.savefig("dog_abnormal_omni_test_summary.png", dpi=150, bbox_inches="tight"); plt.close()
    print("  💾 dog_abnormal_omni_test_summary.png")
...
def test():
    ...
    DOG_SKIN_CLASSES   = ckpt["dog_skin_classes"]
    DOG_EYES_CLASSES   = ckpt["dog_eyes_classes"]
    ...
    print(f"  Skin classes ({len(DOG_SKIN_CLASSES)}):  {DOG_SKIN_CLASSES}")
    print(f"  Eyes classes ({len(DOG_EYES_CLASSES)}): {DOG_EYES_CLASSES}")
    ...
    skin_loader = make_loader(
        InferenceDataset(os.path.join(test_root, "skin"), DOG_SKIN_CLASSES, json_feat_dim))
    eyes_loader = make_loader(
        InferenceDataset(os.path.join(test_root, "eyes"), DOG_EYES_CLASSES, json_feat_dim))
    ...
    print("\n🩺 Skin")
    s_acc, s_per, s_preds, s_labels = evaluate(skin_model, skin_loader, DOG_SKIN_CLASSES, device)
    print(f"  Overall: {s_acc*100:.2f}%")
    for cls, acc in zip(DOG_SKIN_CLASSES, s_per):
        print(f"    {cls:<40s}: {acc*100:.1f}%")

    print("\n👁️  Eyes")
    e_acc, e_per, e_preds, e_labels = evaluate(eyes_model, eyes_loader, DOG_EYES_CLASSES, device)
    print(f"  Overall: {e_acc*100:.2f}%")
    for cls, acc in zip(DOG_EYES_CLASSES, e_per):
        print(f"    {cls:<40s}: {acc*100:.1f}%")
    ...
    print("\n  📈 Confusion Matrix 저장...")
    plot_confusion_matrix(s_preds, s_labels, DOG_SKIN_CLASSES,
                          "Dog Skin — Test (no JSON)", "dog_abnormal_skin_cm.png")
    plot_confusion_matrix(e_preds, e_labels, DOG_EYES_CLASSES,
                          "Dog Eyes — Test (no JSON)", "dog_abnormal_eyes_cm.png")
    save_summary_plot(
        {"skin": {"acc": s_acc, "per_class": s_per},
         "eyes": {"acc": e_acc, "per_class": e_per}},
        {"skin": DOG_SKIN_CLASSES, "eyes": DOG_EYES_CLASSES},
